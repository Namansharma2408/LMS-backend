...
// ---------------------------------------------------------------------------
// MongoDB URI
// Uses the URL constructor to safely swap the database name for the test env.
// Works correctly with both Atlas (mongodb+srv://) and local URIs.
// ---------------------------------------------------------------------------
export const getMongoDBURI = () => {
  const base = process.env.MONGODB_URI || 'mongodb://localhost:27017/nexus';
  ...
  try {
    ...
    // Fallback: regex swap for any non-standard URI shape
    return base.replace(/\/[^/?]+(\?|$)/, '/nexus_test$1');
  }
};

// ---------------------------------------------------------------------------
// Curriculum generator (unchanged)
// ---------------------------------------------------------------------------
    ...
    children: [
      {
        ...
        videoUrl:
          'https://res.cloudinary.com/dk6nbaps1/video/upload/v1782451962/output_144p_mw4twn.mp4',
        duration: '5:00',
        description: `An overview of what we will build and learn in the ${courseName} course.`,
        notes:
          'Welcome! Take a moment to read the prerequisites and download the course resource guide.',
      },
      {
        type: 'lecture',
        ...
        duration: '6:30',
        description:
          'Step-by-step setup guide for compiler/interpreter tools and local environment.',
        notes:
          'Ensure your dependencies are fully installed before starting the first project.',
      },
    ],
  },
  {
    ...
    children: [
      {
        ...
        duration: '8:15',
        description:
          'In-depth understanding of standard APIs, design patterns, and vocabulary.',
      ...
      },
      {
        ...
        duration: '7:45',
        description:
          'Build, debug, and submit your first project for automated scoring.',
        notes:
          'Push your code to Git. A feedback summary will be generated within 10 minutes.',
      },
    ],
  },
];
...
// Seed helpers
// ---------------------------------------------------------------------------
const seedUsers = async (isTest, hashedPassword) => {
  const userCount = await User.countDocuments();
  if (!isTest && userCount >= 10) return;
  ...
  if (!isTest) {
    ...
    const extraUsers = Array.from({ length: 45 }, (_, i) => ({
      name: `${firstNames[i % firstNames.length]} ${lastNames[i % lastNames.length]}`,
      email: `${firstNames[i % firstNames.length].toLowerCase()}.${lastNames[i % lastNames.length].toLowerCase()}${i}@nexus.dev`,
      password: hashedPassword,
      role: i < 5 ? 'instructor' : 'student',
      joinedDate: new Date(Date.now() - i * 3 * 24 * 60 * 60 * 1000),
      purchasedCourseIds: [],
      status: 'Active',
      avatar: '',
      phone: `+1 555-01${10 + i}`,
...
const seedCourses = async (isTest, instructorIds) => {
  const courseCount = await Course.countDocuments();
  if (!isTest && courseCount >= 5) return;
  ...
  const baseCoursesData = [
    {
      ...
      description:
        'Master React server components, concurrent rendering, state management at scale, and high-performance design patterns.',
    ...
    },
    {
      ...
      description:
        'Learn user interview frameworks, usability testing protocols, and SaaS user onboarding optimization strategies.',
      category: 'Design',
      duration: 25,
      level: 'Intermediate',
    },
    {
      ...
      description:
        'A complete guide to multi-gateway routing, ledger reconciliation, subscription billing logic, and payout compliance.',
      category: 'Web Development',
      duration: 18,
      level: 'Beginner',
    },
    {
      ...
      description:
        'Leverage generative AI to script, outline, record, and market high-converting professional online courses.',
      category: 'Web Development',
      duration: 12,
      level: 'Beginner',
    },
  ...
  const coursesToInsert = baseCoursesData.map((c, idx) => ({
    ...c,
    instructors: [instructorIds[idx % instructorIds.length]],
    enrollments: 0,
...
const seedCoupons = async (isTest) => {
  const couponCount = await Coupon.countDocuments();
  if (!isTest && couponCount >= 2) return;
...
const seedEnrollmentsAndTransactions = async (isTest) => {
  const enrollmentCount = await Enrollment.countDocuments();
  const studentsWithNoEnrollments = await User.countDocuments({
    role: 'student',
    purchasedCourseIds: { $size: 0 },
  });

  if (!isTest && enrollmentCount >= 10 && studentsWithNoEnrollments === 0) return;

  console.log('Seeding enrollments & transactions...');
  ...
  // Reset course stats before accumulating
  await Course.updateMany({}, { $set: { enrollments: 0, revenue: 0 } });
  ...
  for (let idx = 0; idx < students.length; idx++) {
    ...
    for (const course of coursesToEnroll) {
      ...
      const certStatus =
        progress === 100 ? 'Issued' : progress > 85 ? 'Pending' : 'In progress';

      const daysAgo = idx * 2 + (idx % 2);
      const recordDate = new Date(
        Date.now() -
          daysAgo * 24 * 3600 * 1000 -
          Math.floor(Math.random() * 24) * 3600 * 1000 -
      ...
      await Course.findByIdAndUpdate(course._id, {
        $inc: { enrollments: 1, revenue: course.price },
      });
    }
  ...
  console.log('Enrollments & transactions seeded successfully.');
};

// ---------------------------------------------------------------------------
// Migrate all lecture videoUrls to the current Cloudinary URL
// ---------------------------------------------------------------------------
  ...
  for (const course of allCourses) {
    ...
    const updateNodes = (nodes) => {
      for (const node of nodes) {
        if (node.type === 'lecture' && node.videoUrl !== CURRENT_VIDEO_URL) {
          node.videoUrl = CURRENT_VIDEO_URL;
          modified = true;
    ...
    if (course.curriculum) updateNodes(course.curriculum);
...
const seedDB = async () => {
  ...
  await seedUsers(isTest, hashedPassword);

  const instructors = await User.find({ role: 'instructor' });
  const instructorIds = instructors.map((i) => i._id);

  // Guard: if no instructors exist yet, fall back to the known seed ObjectId
  if (instructorIds.length === 0) {
    instructorIds.push(new mongoose.Types.ObjectId('60d5ec4b1a4574a2dc907102'));
  }

  await seedCourses(isTest, instructorIds);
  await seedCoupons(isTest);
  await seedEnrollmentsAndTransactions(isTest);
  await migrateVideoUrls();
};

// ---------------------------------------------------------------------------
// Public entry point — call this from index.js and await it before listen()
// ---------------------------------------------------------------------------
export const initDB = async () => {
  mongoose.set('bufferCommands', false);

  if (mongoose.connection.readyState === 1) {
    // Already connected — just seed
    await seedDB();
    return;
  }

  // Disconnect any stale connection (states: 2 = connecting, 3 = disconnecting)
  if (mongoose.connection.readyState !== 0) {
    await mongoose.disconnect();
  }

  const uri = getMongoDBURI();
  console.log(`Connecting to MongoDB (env: ${process.env.NODE_ENV || 'development'})...`);

  await mongoose.connect(uri, {
    serverSelectionTimeoutMS: 10000, // 10 s — generous for Atlas cold starts
...
// ---------------------------------------------------------------------------
// Thin CRUD helpers (unchanged API surface)
// ---------------------------------------------------------------------------
const toPlain = (doc) => ({ ...doc, _id: doc._id.toString(), id: doc._id.toString() });

export const getUsers = async () => (await User.find().lean()).map(toPlain);
export const saveUsers = async (users) => {
  for (const u of users) await User.findByIdAndUpdate(u._id ?? u.id, u, { upsert: true });
};

export const getCourses = async () => (await Course.find().lean()).map(toPlain);
export const saveCourses = async (courses) => {
  for (const c of courses) await Course.findByIdAndUpdate(c._id ?? c.id, c, { upsert: true });
};

export const getTransactions = async () =>
  (await Transaction.find().sort({ date: -1 }).lean()).map(toPlain);
export const saveTransactions = async (transactions) => {
  for (const t of transactions)
    await Transaction.findByIdAndUpdate(t._id ?? t.id, t, { upsert: true });
};

export const getCoupons = async () => (await Coupon.find().lean()).map(toPlain);
export const saveCoupons = async (coupons) => {
  for (const c of coupons) await Coupon.findByIdAndUpdate(c._id ?? c.id, c, { upsert: true });
};

export const getEnrollments = async () => (await Enrollment.find().lean()).map(toPlain);
export const saveEnrollments = async (enrollments) => {
  for (const e of enrollments)
    await Enrollment.findByIdAndUpdate(e._id ?? e.id, e, { upsert: true });
};